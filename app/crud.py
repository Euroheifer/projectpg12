...
def get_settlement_summary(db: Session, group_id: int):
    """Get settlement summary for a group based on all expenses and payments."""
    ...
    for member in members:
        ...
        # Get total amount this user should pay (their share of expenses)
        total_share = db.query(func.sum(models.ExpenseSplit.amount)).join(
        ...
        # Get total payments made by this user
        total_paid = db.query(func.sum(models.Payment.amount)).filter(
            models.Payment.from_user_id == user_id
        ).join(
            models.Expense, models.Payment.expense_id == models.Expense.id
        ).filter(
            models.Expense.group_id == group_id
        ).scalar() or 0
        
        # Calculate balance: positive means should receive money, negative means should pay money
        ...
        if balance < 0:
            total_settlement_amount += abs(balance)
            settlement_count += 1
...
def get_settlement_history(db: Session, group_id: int):
    """Get settlement history for a group."""
    settlements = db.query(models.Settlement).options(
        joinedload(models.Settlement.from_user),
        joinedload(models.Settlement.to_user)
    ).filter(
    ...
    # Transform to include usernames
    result = []
    for settlement in settlements:
        result.append({
            ...
            "from_user_id": settlement.from_user_id,
            "to_user_id": settlement.to_user_id,
            "from_username": settlement.from_user.username if settlement.from_user else None,
            "to_username": settlement.to_user.username if settlement.to_user else None,
            "amount": float(settlement.amount),
            "created_at": settlement.created_at,
            "notes": settlement.notes
        })
    
    return {"settlements": result, "total": len(result)}
