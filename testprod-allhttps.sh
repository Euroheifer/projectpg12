...
call_api() {
  ...
  if [ -z "$4" ]; then
    curl -sS -k -X "$1" "$2" -H "Authorization: Bearer $3"
  else
    curl -sS -k -X "$1" "$2" -H "Authorization: Bearer $3" -H "Content-Type: application/json" -d "$4"
  fi
}
...
for i in {1..6}; do
  ...
  username="user${i}"
  password="pass${i}"
  response=$(curl -s -k -X POST "$API/users/signup" \
...
for i in {1..6}; do
  ...
  password="pass${i}"
  token=$(curl -s -k -X POST "$API/token" \
